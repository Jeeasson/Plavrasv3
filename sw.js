...
const CACHE_AUDIO   = `aprende-audio-${CACHE_VERSION}`;
...
self.addEventListener('activate', event => {
  ...
  const validCaches = [CACHE_STATIC, CACHE_IMAGES, CACHE_FONTS, CACHE_AUDIO];
  event.waitUntil(
    caches.keys()
...
self.addEventListener('fetch', event => {
  ...
  if (url.hostname.includes('puter.com') || url.hostname.includes('js.puter.com')) {
    event.respondWith(cacheFirst(request, CACHE_STATIC));
    return;
  }
});
